...
const novoUsuarioSchema = z.object({
  ...
  documento: z.string().trim().min(11, "Documento inválido").max(20),
...
export const criarUsuario = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (digitos.length !== esperado) {
      throw new Error(data.documento_tipo === "cpf" ? "CPF inválido." : "CNPJ inválido.");
    }
...
export const redefinirSenha = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: unknown) =>
    z.object({ id: z.string().uuid(), senha: z.string().min(8).max(72) }).parse(data),
  )
  .handler(async ({ data, context }) => {
    await exigirAdmin(context.userId);
