...
function validarNovoUsuario(input: NovoUsuario) {
  ...
  // Administrador não precisa de documento — só aluno.
  if (input.role === "admin") return;
 
  if (!input.documento_tipo) {
    throw new Error("Informe o tipo de documento (CPF ou CNPJ).");
  }
...
export async function criarUsuarioAdmin(input: NovoUsuario, solicitanteId: string) {
  ...
  const empresa = ehCnpj && input.documento ? await razaoSocialPorCnpj(input.documento) : null;
  if (ehCnpj && !empresa) {
